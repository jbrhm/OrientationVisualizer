...
private:
    ...
    // Vector to keep track of all windows that GLFW will have to destroy upon termination
...
public:
    ...
    inline static WindowPtr createWindow(int width, int height, const std::string& name, GLFWmonitor* monitor = nullptr, GLFWwindow* share = nullptr){
        GLFWwindow* ptr = glfwCreateWindow(width, height, name.c_str(), monitor, share);
        if (!ptr) {
